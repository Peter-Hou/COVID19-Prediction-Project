...
Starting from this point, the rest of the report will focus on Canada; but the code can be generalized for any countries. It is just Canada is the target country for analysis in this report.
...
plot(t_Canada_consolidate$date, t_Canada_consolidate$cured,  main = "COVID-19 Cases in Canada from 2020-01-22 to 2020-04-20", xlab = "Date", ylab = "Number of Cases", xaxt = "n", col = "navyblue", lty = 1, pch = 3)
...
Note that our estimated reproductive number is very high compared to any known epidemic occured in the past, which usually have an average of below 5. 

The reason for such a high value of the estimate is mainly attribute to the lack of sufficient data. Reproductive number represent how many people will be infected by any one infected people. From our graph, we can see that the values of the confirmed cases skyrocketed to at the end of our data collection periods. Before that, the cases are leveled at 0. This dramatic, sudden increase is what causes the large estimate of the reproductive number.

However, the confirmed cases leveled at 0 does not mean that there are no confirmed cases. It only means that no confirmed cases have chosen to take a test and find out they are positive. The initial symptom for youngsters are mild; which many people may not take a serious attention. 

Hopefully, the true reproductive number should be much smaller. But we can only learn that after we have gained more comprehensive data.


## Predictive Forecast

There is another dataset which involves detailed record of a person who is infected.
