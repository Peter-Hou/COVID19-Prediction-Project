---
title: "Analysis with COVID-19 situation in China"
author: "Peter Hou"
date: "03/05/2020"
output:
...
Starting from this point, the rest of the report will focus on China; but the code can be generalized for any countries. It is just China is the target country for analysis in this report.

We now examine the consolidated data for country **China** as an example

``` {r}
china_consolidate <- consolidate_data_by_country("China")

china_consolidate[, 1:5]
```
...
``` {r}
t_china_consolidate <- t(china_consolidate)
head(t_china_consolidate)
t_china_consolidate <- t_china_consolidate[-1, ] # the row of country is not needed
rn <- as.Date(rownames(t_china_consolidate), "X%m.%d.%y")
head(t_china_consolidate)
t_china_consolidate = data.frame(t_china_consolidate)
t_china_consolidate = cbind(date = rn, t_china_consolidate)

rownames(t_china_consolidate) <- as.character(as.Date(rownames(t_china_consolidate), "X%m.%d.%y"), origin = "2020-01-22")

str(t_china_consolidate)

t_china_consolidate$confirmed <- as.integer(t_china_consolidate$confirmed)
t_china_consolidate$death <- as.integer(t_china_consolidate$death)
t_china_consolidate$cured <- as.integer(t_china_consolidate$cured)
```
...
plot(t_china_consolidate$date, t_china_consolidate$cured,  main = "COVID-19 Cases in China from 2020-01-22 to 2020-04-20", xlab = "Date", ylab = "Number of Cases", xaxt = "n", col = "navyblue", lty = 1, pch = 3)
axis(1, at = t_china_consolidate$date[which(t_china_consolidate$date %in% date_for_xaxis)], labels = c("Feb", "Mar", "Apr"))
lines(t_china_consolidate$date, t_china_consolidate$confirmed,  col = "orange", lty = 2, pch = 13)
lines(t_china_consolidate$date, t_china_consolidate$death,  col = "red", lty = 3, pch = 16)
...
reshaped_consolidate_data <- melt(t_china_consolidate, id.vars="date")

reshaped_consolidate_data

p1 <- ggplot(reshaped_consolidate_data, aes(date, value, col=variable)) +
    geom_point() +  ggtitle("China COVID-19 Record") + labs(y = "Cases Number", x = "Date", color = "Case Type") 
...
1. $\lamabda$ is the uniform probability of being infected, note that it is not constant. It must consider other factors like the spreadness of the virus. E.g. if everyone around you has been infected, then there is a larger change that you get infected than an average person.
...
``` {r}
recover_sample <- t_china_consolidate$Cured
...
poiLF <- function(theta) {exp(-length())}
